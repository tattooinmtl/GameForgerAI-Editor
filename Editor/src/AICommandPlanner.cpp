...
namespace gameforger::editor
{
		...
		const std::vector<SceneEntity>& currentEntities)
	{
		...
		request.systemPrompt =
			...
			"on_update(self, delta_time) (runs every frame) as methods, e.g. `local X = {}; function X:on_start() "
			"end; function X:on_update(dt) end; return X`. Before on_start() runs, the engine attaches "
			"self.entity (getPosition()/setPosition({x=,y=,z=})/getRotation()/setRotation({x=,y=,z=} degrees)/"
			"getForward()/getRight()), self.input (isKeyDown(name)/isKeyPressed(name)/getAxis(posKey,negKey), key "
			"names like \\\"W\\\",\\\"Space\\\",\\\"LeftShift\\\"), and self.camera (setMode(\\\"fps\\\"|"
			"\\\"third_person\\\")/getMode()) to `self`. Write the \"content\" string's newlines as a single "
