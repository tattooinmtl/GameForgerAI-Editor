...
| 4.6 | ~~AI-generated scripts get user-typed names, with no convention.~~ **FIXED for new scripts** by `suggestedScriptFileName`, which names them `<Purpose>_<GameTitle>.lua`. The 14 existing orphans are untouched - pruning them is content, not code. Original finding: `Game/Scripts/` already holds `FPSController.lua`, `PlayerFPS.lua`, `fps_controller.lua`, `player_fps.lua`, `controller.lua`, `player_controller.lua`, `Script.lua`, `Script_2.lua`, `test.lua` — nine files, several of them the same thing. | MISSING | `ls Game/Scripts/`; `scriptCreator.scriptName`, `main.cpp:6527` | 4 |
...
| 4.7 | ~~The AI prompt is not in the script window, can only create, and cannot modify.~~ **FIXED.** A dockable **Scripts panel** now holds the file list, a real (non-modal) editor, attach/detach for the selection, and the AI prompt at the bottom with a Create/Modify switch. `modifyEntityScript` sends the existing source plus an instruction and returns the whole new file - whole-file rather than a patch because this project has no diff-apply machinery, and a half-applied patch leaves a script that neither loads nor resembles what the user had. Generated scripts are auto-named `<Purpose>_<GameTitle>.lua`. The old Inspector path is **gone**: the "Add Script" and "Edit Script" modals, `ScriptCreatorState`, `ScriptEditorState` and their naming helpers were deleted (-500 lines), the 14 presets moved into the Scripts panel as a collapsible section, and every remaining entry point - the Inspector's `Edit`/`Add Script...`, the Hierarchy right-click, the Project browser's `.lua` click - now raises the panel via `ScriptsPanelState::requestOpenPath` instead. Two defects died with it: a script could be open in a modal and in the panel with two different copies of its text, and applying *any* preset silently set `Collider` (so an Audio Manager on an empty blocked the player) - `marksCollider` is now per-preset data matching each description. | FIXED | 4 |
